...
class _OperationKey:
    __or_operator_in_contains_part_regex = re.compile(r'contains\(.*\|.*\)')
    ...
    def __or_operator_present_in_main_part_xpath_1(self):
        xpath_without_contains_part = re.sub(self.__or_operator_in_contains_part_regex, '', self.xpath_1)
        return True if '|' in xpath_without_contains_part else False

    def __or_operator_present_in_main_part_xpath_2(self):
        xpath_without_contains_part = re.sub(self.__or_operator_in_contains_part_regex, '', self.xpath_2)
        return True if '|' in xpath_without_contains_part else False
